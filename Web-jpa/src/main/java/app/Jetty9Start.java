...
/**
 * @author WH
 * @ClassName: Jetty9Start
 * @Description: Jetty9嵌入式启动
 * @date 2019-3-06 下午16:03:19
 */
public class Jetty9Start {
    public static final int PORT = 80;
    public static final String CONTEXT = "/";
    //    private static final String DEFAULT_WEBAPP_PATH = "src/main/webapp";
    private static final String DEFAULT_WEBAPP_PATH = "Web-jpa/src/main/webapp";
    ...
    public void startJetty(int port, String context, String env) {
        Server server = null;
        if ("dev".equals(env)) {
            server = Jetty9Start.createDevServer(port, context);
        } else {
            server = Jetty9Start.createProdServer(port, context);
        }
        try {
            server.stop();
            server.start();
    ...
    public static void main(String[] args) {
        new Jetty9Start().startJetty(PORT, CONTEXT, "dev");//dev为开发模式
    }
    ...
    public static Server createProdServer(int port, String contextPath) {
        ...
        ProtectionDomain protectionDomain = Jetty9Start.class.getProtectionDomain();
